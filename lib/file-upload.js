  ...
  if (error) {
    const err = new Error(
      error.message?.includes("Bucket not found")
        ? `Storage bucket '${STORAGE_BUCKET}' not found — Supabase에서 버킷을 생성하세요.`
...
async function writeTempFile(buffer, fileName) {
  const tmpPath = path.join(
    os.tmpdir(),
    `gemini-${Date.now()}-${safeFileName(fileName)}`
  );
...
async function uploadBufferWithSdk(buffer, mimeType, displayName, apiKey) {
  const tmpPath = await writeTempFile(buffer, displayName || "upload");
  try {
