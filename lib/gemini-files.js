...
async function startGeminiUpload({ bytes, mimeType, displayName, apiKey }) {
...
async function uploadGeminiChunk({ uploadUrl, offset, buffer, finalize }) {
  ...
  if (!finalize) {
    if (![200, 201, 308].includes(uploadRes.status)) {
      ...
      const err = new Error(data?.error?.message || `Gemini chunk upload failed (${uploadRes.status})`);
      err.status = 502;
      throw err;
    }
    return { ok: true, finalize: false, bytes: buffer.length };
  }
  ...
  return { ok: true, finalize: true, file };
}
...
async function beginGeminiFileUpload({ fileName, mimeType, size }) {
  const apiKey = requireApiKey();
  const { resolvedMime } = validateFileMeta({ fileName, mimeType, bytes: size });
...
async function analyzeGeminiFileRef({ fileUri, geminiFileName, mimeType, fileName, prompt }) {
  const apiKey = requireApiKey();
  const { resolvedMime } = validateFileMeta({ fileName, mimeType, bytes: 1, checkSize: false });
  const instruction = requirePrompt(prompt);

  if (!fileUri || !geminiFileName) {
    const err = new Error("fileUri and geminiFileName are required");
    err.status = 400;
    throw err;
  }
...
async function analyzeFileBuffer({ buffer, mimeType, fileName, prompt }) {
  ...
  const { resolvedMime, instruction } = (() => {
    const meta = validateFileMeta({ fileName, mimeType, bytes: buffer.length });
    return { ...meta, instruction: requirePrompt(prompt) };
  })();
...
module.exports = {
  analyzeFileBuffer,
  beginGeminiFileUpload,
  analyzeGeminiFileRef,
  uploadGeminiChunk,
  getGeminiApiKey,
  GEMINI_MODEL,
  ALLOWED_MIME,
