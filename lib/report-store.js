...
async function saveReport({ topic, title, summary, content, sources, source_type = "search" }) {
  ...
  if (!row.topic || !row.title) {
    ...
    const err = new Error(`보고서 저장 실패: ${missing.join(", ")} 필수`);
    err.status = 400;
    throw err;
  }
...
async function getReportById(id) {
  ...
  if (error) {
    const err = new Error(error.message || "Failed to load report");
    err.status = 502;
    throw err;
  }
...
module.exports = { saveReport, listReports, getReportById, bodyAsMarkdown };
